...
function Stage:SpawnRoom()
	...
	local function createContent()
        ...
        pyramid1:PivotTo(CFrame.new(-(roomPivot.X + roomSize.X / 2) + 1 + size.X / 2, (roomPivot.Y - roomSize.Y / 2) + 1 + size.Y / 2, roomPivot.Z))
		local pyramid2 = createPyramid()
        local _, size = pyramid2:GetBoundingBox()
        pyramid2:PivotTo(CFrame.new((roomPivot.X + roomSize.X / 2) - 1 - size.X / 2, (roomPivot.Y - roomSize.Y / 2) + 1 + size.Y / 2, roomPivot.Z))
	end
